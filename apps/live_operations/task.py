...
def sync_claim_statuses():
    ...
    try:
        # Get all claims that are in live database
...
@shared_task(name='apps.live_operations.tasks.push_pending_claims_to_live_with_filter')
def push_pending_claims_to_live_with_filter(status_filter=None, days_old=None):
    """
    ...
    try:
        ...
        claim_ids = claims.values_list('id', flat=True)
        ...
        result = LiveDatabaseService.push_claims_by_ids(list(claim_ids))
        
        logger.info(f"Filtered push completed: {result}")
        return result
        
    except Exception as e:
        logger.error(f"Error in push_pending_claims_to_live_with_filter task: {e}")
        raise
